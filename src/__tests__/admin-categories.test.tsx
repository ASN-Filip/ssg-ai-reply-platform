...
describe('Admin categories CRUD UI', () => {
  ...
  const electronicsMatches = await screen.findAllByText('Electronics')
  ...
  expect(screen.getByPlaceholderText(/Category name/i)).toBeTruthy()
  })
  ...
  renderWithClient(<CategoriesClient />)
  const electronicsMatches2 = await screen.findAllByText('Electronics')
  expect(electronicsMatches2.length).toBeGreaterThan(0)

    fireEvent.click(screen.getByText(/Add category/i))
    await waitFor(() => expect(screen.getByRole('dialog')).toBeTruthy())

  fireEvent.change(screen.getByPlaceholderText(/Category name/i), { target: { value: 'New Cat' } })
  fireEvent.change(screen.getByPlaceholderText(/e.g. Samsung NL/i), { target: { value: 'New Label' } })
